...
public class Main {
	...
	private static final String KPWR = "kpwr_wsd_blogi";
	private static final String CSV = "csv_blogi";
	private static final String DICT = "dict_blogi.csv";
...
//		params = new BayesParams(args.toArray(new String[args.size()]));
		...
		DocumentMap map = ObjectIOManager.load(new File("rawSynsetBlogi2.map"));
//		
		DocumentLoader loader = new DocumentLoader(new File(KPWR), new File(CSV));
//		List<Document> docs =loader.loadDocuments();
//		
//		DocumentMap.createVectors(new File("synset.pca"), new File(DICT), docs);
		
		List<Document> documents = loader.loadDocuments();
		List<int[]> list = map.mapDocuments(documents, new File(DICT));
